...
with open("./resources/nasdaq daily.csv", newline="") as csvFile:
...
with open("./resources/nyse daily.csv", newline="") as csvFile:
    insert(csv.reader(csvFile, delimiter=",", quotechar="|"), db_connection,
           {0, 1, 4, 6}, "NYSE_COMPOSITE_DAILY", {}, {0})

execute(db_connection, election_data)
with open("./resources/election_by_state.csv", newline="") as csvFile:
    insert(csv.reader(csvFile, delimiter=",", quotechar="|"), db_connection,
           {0, 1, 2, 3, 4}, "PRESIDENTIAL_ELECTION_BY_STATE", {}, {0})

execute(db_connection, unemployment_data)
with open("./resources/State_Unemployment_Rate.csv", newline="") as csvFile:
    insert(csv.reader(csvFile, delimiter=",", quotechar="|"), db_connection,
           {0, 1, 2, 3}, "STATE_UNEMPLOYMENT_RATE", {}, {0, 1})

execute(db_connection, state_data)
with open("./resources/US_States.csv", newline="") as csvFile:
    insert(csv.reader(csvFile, delimiter=",", quotechar="|"), db_connection,
           {0, 1, 2, 3, 4}, "US_STATES", {}, {0})

execute(db_connection, covid_data)
with open("./resources/COVID_Cases.csv", newline="") as csvFile:
    insert(csv.reader(csvFile, delimiter=",", quotechar="|"), db_connection,
